...
public_users.post("/register", (req,res) => {
    ...
    if(userExists)
    {
        return res.status(401).json({message : "username already in use , please use different username"});
    }
    users.push({
        username,
        password
    });
    return res.status.send(201).json({message : "user created successfully"});
